...
export function ConfiguracionClient({ clinicaInicial, adminId, adminEsDoctor, adminEspecialidad, modulosActivosInicial, esParticular = false }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="space-y-3">
            {([
              { key: 'recordatorioSMS' as keyof NotifConfig,   label: 'Recordatorio por SMS',           desc: 'Envía un SMS antes de cada cita' },
              { key: 'recordatorioEmail' as keyof NotifConfig, label: 'Recordatorio por email',          desc: 'Envía un email antes de cada cita' },
              { key: 'confirmacionCita' as keyof NotifConfig,  label: 'Confirmación de reserva',         desc: 'Email al confirmar una nueva cita' },
              { key: 'cancelacionCita' as keyof NotifConfig,   label: 'Aviso de cancelación',            desc: 'Email cuando se cancela una cita' },
              ...(!esParticular ? [{ key: 'resumenDiario' as keyof NotifConfig, label: 'Resumen diario al profesional', desc: 'Email a cada profesional con su agenda del día' }] : []),
            ]).map(({ key, label, desc }) => (
      ...
      {/* ── Sección 5: Perfil de profesional del admin (oculta para particular) ── */}
      {!esParticular && <section className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 bg-slate-50/60">
