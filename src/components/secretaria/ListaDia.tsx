...
export function ListaDia({ citas, showMedico = false }: ListaDiaProps) {
  if (citas.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <button className="mt-6 flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl transition-colors">
  ...
  return (
    <div className="space-y-1.5">
      {window.map((slot) => {
        ...
        if (cita) {
          return <CitaCard key={cita.id} cita={cita} showMedico={showMedico} />
        }
        ...
        return (
          ...
          >
            ...
            <button className="ml-auto w-5 h-5 flex items-center justify-center rounded-md bg-slate-200 group-hover:bg-blue-500 text-slate-500 group-hover:text-white transition-all">
